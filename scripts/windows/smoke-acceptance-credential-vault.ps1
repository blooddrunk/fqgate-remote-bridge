...
$modernDummy = ConvertTo-SecureString ("cfast_" + ("A" * 48)) -AsPlainText -Force
...
try {
    foreach ($target in $targets) {
        $value = if ($target -eq $targets[2]) { $modernDummy } else { $dummy }
        [FQGateAcceptanceCredential]::Write($target, $value, "disposable-test", $ownerSid)
