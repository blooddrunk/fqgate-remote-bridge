...
Stable plan action set: `noop`, `adopt`, `create`, `update`, `manual_block`,
`unsafe_conflict`.
...
Add explicit read-only commands such as:

```text
cloudflare setup inspect --json --config <path>
cloudflare setup plan --json --config <path>
```
...
exact-final-commit CI, and no unresolved `manual_block` or `unsafe_conflict`. Safe unambiguous
